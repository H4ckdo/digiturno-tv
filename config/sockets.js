...
module.exports.sockets = {
  ...
  ***************************************************************************/
  afterDisconnect: function(session, socket, cb) {
    const id = session.moduloId;
    debugger;
    if(id) {
      Modulos.update({ id }, { online: false }).then(docs => {
        let neverCalleds = Tokens.update({ dispatchedBy: id, dispached: false, called: false, canceled: false }, { canceled: true })

        neverCalleds.then((tokens) => {
          let noComplete = Tokens.update({ dispatchedBy: id, dispached: false, canceled: false, called: true }, { canceled: true })
          noComplete.then((nor) => {
            sails.sockets.broadcast("TV", "pullToken", {data: nor})
            cb()
